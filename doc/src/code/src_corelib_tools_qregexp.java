/*   Ported from: src.corelib.tools.qregexp.cpp
<snip>
//! [0]
        QRegExp rx("(\\d+)");
...
//! [0]
        QRegExp rx("(\\d+)");
        Stringsstr = "Offsets: 12 14 99 231 7";
        List<String> list;
        int pos = 0;
...
//! [7]
        Stringscaptured = rx.cap(1); // captured == "letter"
//! [7]


//! [8]
        QRegExp rx("&(?!amp;)");      // match ampersands but not mp;
        Stringsline1 = "This & that";
        line1.replace(rx, "mp;");
        // line1 == "This mp; that"
        Stringsline2 = "His mp; hers & theirs";
        line2.replace(rx, "mp;");
...
//! [9]
        Stringsstr = "One Eric another Eirik, and an Ericsson. "
                      "How many Eiriks, Eric?";
        QRegExp rx("\\b(Eric|Eirik)\\b"); // match Eric or Eirik
        int pos = 0;    // where we are in the string
        int count = 0;  // how many Eric and Eirik's we've counted
...
//! [13]
        Stringsstr = "offsets: 1.23 .50 71.00 6.00";
        QRegExp rx("\\d*\\.\\d+");    // primitive floating point matching
        int count = 0;
        int pos = 0;
        while ((pos = rx.indexIn(str, pos)) != -1) {
...
//! [17]
        ...
        int pos = rxlen.indexIn("Length: 189cm");
        if (pos > -1) {
            Stringsvalue = rxlen.cap(1); // "189"
            Stringsunit = rxlen.cap(2);  // "cm"
            // ...
        }
//! [17]
