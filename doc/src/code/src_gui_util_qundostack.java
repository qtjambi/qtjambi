/*   Ported from: src.gui.util.qundostack.cpp
<snip>
//! [0]
...
//! [0]
    ...
    {
    public:
        AppendText(Stringsoc, Stringsext)
            : m_document(doc), m_text(text) { setText("append text"); }
        virtual void undo()
            { m_document.chop(m_text.length()); }
        virtual void redo()
...
//! [1]
    MyCommand ommand1 = new MyCommand();
    stack.push(command1);
    MyCommand ommand2 = new MyCommand();
    stack.push(command2);

    stack.undo();

    MyCommand ommand3 = new MyCommand();
    stack.push(command3); // command2 gets deleted
//! [1]
...
//! [3]
    booleansAppendText.mergeWith(QUndoCommand ther)
    {
        if (other.id() != id()) // make sure other is also an AppendText command
            return false;
        m_text += static_cast<AppendText*>(other).m_text;
        return true;
    }
//! [3]
