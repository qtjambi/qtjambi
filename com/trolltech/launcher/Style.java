...
public class Style extends QWindowsStyle
{
    ...
    public static boolean isOn(int state) { return (state & QStyle.State_On) != 0; }
    public static boolean isSunken(int state) { return (state & QStyle.State_Sunken) != 0; }
    public static boolean isEnabled(int state) { return (state & QStyle.State_Enabled) != 0; }

    public void drawComplexControl(int cc, QNativePointer opt, QPainter p, QWidget widget) {
	switch (cc) {
    ...
    public void drawControl(int cc, QNativePointer opt, QPainter p, QWidget widget) {
	switch (cc) {
	...
	case QStyle.CE_PushButton:
	    ...
	    break;
	default:
	    super.drawControl(cc, opt, p, widget);
	    break;
	}
    }
	...
	QRectF text_rect = new QRectF(p.boundingRect(ROUND, 0,
						     rect.width() - ROUND * 2,
						     rect.height(),
						     Qt.AlignTop | Qt.AlignHCenter, title));

	text_rect = text_rect.adjusted(-H_TEXT_PADDING,
				       GROUPBOX_LINE_THICKNESS / 2.0,
				       TEXT_PADDING * 2 + H_TEXT_PADDING,
				       TEXT_PADDING * 2);

	drawShadeButton(p, text_rect, state);

	p.setPen(new QPen(new QBrush(TROLLTECH_GREEN), GROUPBOX_LINE_THICKNESS));
	p.drawRoundRect(text_rect,
			(int) (ROUND * 100 / text_rect.width()),
    ...
    public int pixelMetric(int pm, QNativePointer option, QWidget widget) {
	...
	if ((opt.state() & QStyle.State_On) != 0) {
	    p.setBrush(new QBrush(TROLLTECH_GREEN));
	    p.setPen(QPen.NoPen);
	    p.drawEllipse(circle_rect.adjusted(6, 6, -6, -6));
	}

	QRectF text_rect = new QRectF(dim + DOT_TO_TEXT_PADDING,
				      2,
				      rect.width() - dim - DOT_TO_TEXT_PADDING,
				      rect.height());
	p.setPen(PEN_BLACK);
	...
	int state = opt.state();
	boolean button_down = (state & State_Sunken) != 0;
	boolean enabled = (state & State_Enabled) != 0;
	...
	drawShadowText(p, rect.translated(translation, translation), opt.text(),
		       shadow_offset, shadow_offset);

    }


    public static void drawButtonOutline(QPainter p, QRectF rect, int state) {
	p.save();
	if (isEnabled(state))
	    p.setPen(PEN_THICK_GREEN);
	else
	...
	p.drawRoundRect(rect.adjusted(1, 1, -1, -1),
			(int) (ROUND * 100 / rect.width()),
			(int) (ROUND * 100 / rect.height()));
	p.restore();
    }


    public static void drawShadeButton(QPainter p, QRectF rect, int state) {
	p.save();
	p.translate(rect.topLeft());
	p.scale(rect.width(), rect.height());
	p.setBrush(new QBrush(isSunken(state) ? GRADIENT_DARK : GRADIENT_LIGHT));
	p.setPen(QPen.NoPen);
	p.drawRoundRect(0, 0, 1, 1,
			(int) (ROUND * 100 / rect.width()),
			(int) (ROUND * 100 / rect.height()));
    ...
    public static void drawShadowText(QPainter p, QRectF rect, String text,
				      double dx, double dy) {
