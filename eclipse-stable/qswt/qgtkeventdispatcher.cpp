...
GdkFilterReturn QGtkEventDispatcher::gdkXEventProc(GdkXEvent *xevent, GdkEvent *event, gpointer data)
{
    ...
    if (GtkWidget *gtkw = gtk_grab_get_current()) {
        // check if the top level window contains the active qt widget, if it does,
        // handle the events as normal.
        switch (e->type) {
            case ButtonPress:
            case ButtonRelease:
            case MotionNotify:
            ...
            case EnterNotify:
            case LeaveNotify: {
                GdkWindow *grabtoplevel = gtkw->window;
                GdkWindow *aqtoplevel = 0;

                // check if the event targets a qt widget
                QX11EmbedWidget *ew = 0;
                if (QWidget *w = QWidget::find(((XEvent *)xevent)->xany.window))
                    ew = qobject_cast<QX11EmbedWidget *>(w->window());

                // if it is a qt widget, check if it's in the window which has grabbed the input
                if (ew) {
                    gpointer p = gdk_xid_table_lookup(ew->containerWinId());
                    aqtoplevel = gdk_window_get_toplevel((GdkWindow *)p);
                    if (!aqtoplevel || (aqtoplevel != grabtoplevel)) {
